// sections/Industrialization/Drafts.jsx
import { useNavigate } from 'react-router-dom';
import { useState, useEffect } from 'react';
import TableComponent from '../../components/layout/TableComponent';
import { getIndustrializationDrafts, approveRFQInd } from '../api';
...
export default function Drafts() {
    ...
    const [actionLoading, setActionLoading] = useState(null); // rfq id currently being actioned
    ...
    const reload = () => {
        setLoading(true);
        getIndustrializationDrafts().then(rfqs => {
    ...
    const handleSendToPurchases = async (rfqId) => {
        ...
        try {
            await approveRFQInd(rfqId, true);
            reload();
        } catch (err) {
            alert(err.message);
        } finally {
    ...
    const handleDiscardApproval = async (rfqId) => {
        setActionLoading(rfqId);
        try {
            await approveRFQInd(rfqId, false);
            reload();
        } catch (err) {
            alert(err.message);
        } finally {
